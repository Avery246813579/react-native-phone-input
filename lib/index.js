...
export default class PhoneInput extends Component {
  ...
  componentDidMount() {
    if (this.props.textStyle.placeholder) {
      this.setState({formattedNumber: this.state.formattedNumber + ' ' + this.props.textStyle.placeholder});
    }
  }
  ...
  updateFlagAndFormatNumber(number, actionAfterSetState = null) {
    ...
    this.setState({iso2, formattedNumber: this.format(phoneNumber), value: phoneNumber}, actionAfterSetState);
  }
  ...
  render() {
    ...
    let textStyle = JSON.parse(JSON.stringify(this.props.textStyle));
    if (typeof formattedNumber !== "undefined" && formattedNumber.indexOf(textStyle.placeholder) !== -1 && textStyle) {
    ...
    return (
        <View style={[styles.container, this.props.style]}>
          ...
          <View style={{flex: 1, marginLeft: this.props.offset || 10, ...this.props.textWrapperStyle}}>
            <TextInput
                onFocus={() => {
                  if (formattedNumber.indexOf(textStyle.placeholder) !== -1) {
                    this.setState({formattedNumber: countryData ? `+${countryData.dialCode} ` : ""});
                  }
                }}
                ref={ref => {
                  this.inputPhone = ref;
